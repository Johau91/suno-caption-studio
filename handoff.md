...
## 2026-07-10 안정성 보강
- 사용되지 않던 Suno 페이지 설정 패널 제거, 단일 곡 성공·오류를 화면 토스트로 표시
- Suno 페이지 다운로드/일괄 다운로드 메시지 5개 언어 적용
- 결제 주문 ID를 24시간 로컬 보관해 옵션 페이지 재실행 시 자동 활성화 재개
- 결제·주문·라이선스 API 요청 타임아웃 추가
- webwoori: 결제 취소/요청 실패 상태 기록, pending 24시간·failed 7일 자동 정리, 이메일 검증 및 페이앱 타임아웃 추가
- `npm test` 추가: LRC/SRT 시간 포맷, 가사 정리, 파일명 정리, 타임라인 정규화, i18n 보간, 한글 ZIP 파일명 검증

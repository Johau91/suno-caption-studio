...
Suno Caption Studio는 Suno 곡 페이지에서 동기화된 가사/캡션을 미리보고 `LRC`, `SRT`, `TXT`로 저장하거나 클립보드에 복사하는 Chrome 확장 프로그램입니다.

이 프로젝트는 특정 기존 저장소의 수정 포크가 아니라 새로 작성한 독립 코드베이스입니다. Suno와 제휴, 후원, 승인 관계가 없는 비공식 도구입니다.

## 편의 기능

- 곡 페이지에서 자동으로 열리는 플로팅 패널
- 내보내기 전에 `LRC`, `SRT`, `TXT` 미리보기
- 선택한 형식 저장
- 선택한 형식 복사
- `LRC/SRT/TXT` 한 번에 저장
...
게시 전 실제 Chrome Web Store 개발자 계정에 맞는 공개 개인정보처리방침 URL을 준비하세요. 자세한 초안은 [`PRIVACY.md`](PRIVACY.md)를 참고하세요.

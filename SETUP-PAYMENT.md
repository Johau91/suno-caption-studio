...
[구매자] ──결제──> [페이앱] ──웹훅──> [webwoori /api/payapp/feedback] ──발급──> 라이선스 + 이메일(Resend)
```
...
- 가격/기간은 `webwoori/src/lib/licenses.ts`(`LICENSE_TIERS`)와 `src/components/PurchasePanel.tsx`가 소스. 둘을 같이 수정.
...
- `options.js` → 구매 버튼이 `https://webwoori.com/buy/suno-caption` 열기, `meta.app` 검증
...
```
그다음:
1. **webwoori 배포** (git push → 자동 빌드). `https://webwoori.com/buy/suno-caption` 열리는지, `/api/health` OK 확인.
2. **확장 배포/새로고침** (이미 webwoori를 가리킴).
3. **실결제 테스트**: `/buy/suno-caption` → 소액(1개월 ₩2,900) 결제 → 완료 페이지에서 키 확인 → 확장 옵션에 입력 → 프리미엄 확인.
